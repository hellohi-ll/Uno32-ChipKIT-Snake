	...
	*t &= 0xff00; // *Set bits 0-7 as output {TRISE}*
	...
	while(1){ // Infinite program loop
	  if(dead = 2){ // MENU
	    ...
	    if(choise1 && (getsw()&0x1)){ // Choice of game mode
	      playernumber = choise1;
	      if(choise1 == 3){//if check score chosen to go scoreboard
					scoreboard();
					continue;
	      }
	      ...
	      if(choise2 && (getsw()&0x1)) { // Choice of difficulty
					difficulty = choise2;
					dead = 0; // Start the game
	      }
	    }
	  }
	  if(!dead){ // Proceed to game
	    snakeinit();
	  ...
	  if(dead == 1){ // This part handles the score after the game ends
	    who_won();
	    quicksleep(3000000);
	    while(!getbtns()){
	    	who_won();
