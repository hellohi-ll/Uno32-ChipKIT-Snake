...
void generate_food(int boardNumber) {
  ...
  if (boardNumber == 1) {
    generated = 0;
    while(!generated) { // Avoid generating on snake/obstacles, randomize until correct spot found
...
void generate_obstacles(int boardNumber, int obstacleNumber) {
  ...
  int count = 0;
  if (boardNumber == 1) {
    while (count < obstacleNumber){ // How many obstacles ::
      ...
      if (tempRow < 12 || tempRow > 16 || tempColumn < 12 || tempColumn > 17) {
        if (random == 1){ // Draw vertical obstacle
...
void check_collision(int r, int c, int snakeNumber) {
  ...
  else if (read_pixel(r, c)) { // End the game in case of collision with other objects
    if(snakeNumber ==1)
      dead1 = 1;
    else
      dead2 = 1;
